...
class ArcFace(LightningModule):
    def __init__(self, out_features = 13938):
        super(ArcFace, self).__init__()
        self.backbone = resnet_model
        self.backbone.fc = nn.Linear(in_features=512, out_features=128)
        self.header = ArcMarginProduct(in_features=128, out_features=out_features, s=30, m=0.5)
...
def main():
    ...
    trainer = Trainer(accelerator='gpu' if torch.cuda.is_available() else 'cpu', gpus=[opt.device_id], max_epochs=15, gradient_clip_val=5, callbacks=[ModelCheckpoint(monitor="val_acc")])
